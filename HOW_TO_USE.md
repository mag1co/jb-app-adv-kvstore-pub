...
- **Global Admin**: Go to **YouTrack Admin → Adv.KVStore** to manage global blocks, browse project scopes, and check ACL status.
- **Project Admin**: Go to **Project Settings → Adv.KVStore** to manage blocks exclusively for that specific project.
...
Copy [`example_workflow/kvhelper.js`](example_workflow/kvhelper.js) and [`example_workflow/config.js`](example_workflow/config.js) into your workflow package, set your URL and token in `config.js`, then:
...
See also [`example_workflow/example-usage.js`](example_workflow/example-usage.js) for a complete workflow demo and [`example_workflow/test-kvstore.js`](example_workflow/test-kvstore.js) for integration tests.
