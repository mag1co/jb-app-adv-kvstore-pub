 ...
 * Copy this file into your workflow package and add config.js with TOKEN.
 *
 * Usage:
...
var YOUTRACK_BASE_URL = config.YOUTRACK_BASE_URL;
...
function conn() {
  if (!YOUTRACK_BASE_URL || !YOUTRACK_TOKEN) {
    throw new Error('KVHelper: YOUTRACK_BASE_URL and YOUTRACK_TOKEN must be set in config.js');
  }
  if (!_conn) {
    _conn = new http.Connection(YOUTRACK_BASE_URL);
