 ...
 *   - Задан YOUTRACK_TOKEN в config.js (URL определяется из issue.url)
 *
...
const YOUTRACK_BASE_URL = config.YOUTRACK_BASE_URL || '';
const YOUTRACK_TOKEN = config.YOUTRACK_TOKEN;

const BASE = '/api/extensionEndpoints/axkvstore/be';
// Project scope base URL формируется динамически с projectId
const TEST_BLOCK = '__test_kvstore__';
const TEST_KEY = 'testKey';
const TEST_VALUE = 'hello-kvstore';
...
function parseResp(resp) {
  ...
  if (!resp.isSuccess) {
    var body = '';
    try { body = (resp.response || '').substring(0, 300); } catch (_e) { body = '(unreadable)'; }
  ...
  return [
    ...
    // 5. TTL: запись нескольких ключей с TTL
    {
      ...
      fn: function (conn) {
        ...
        var found = null;
        for (i = 0; i < blocks.blocks.length; i++) {
...
exports.rule = entities.Issue.action({
  ...
  action: function (ctx) {
    if (!YOUTRACK_BASE_URL || !YOUTRACK_TOKEN) {
      workflow.check(false, 'KVStore Test: Set YOUTRACK_BASE_URL and YOUTRACK_TOKEN in config.js');
      return;
    }

    var conn = new http.Connection(YOUTRACK_BASE_URL);
